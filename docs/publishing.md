...
`packages/` is the source of truth for shared runtime code. Most skills consume shared packages from npm with semver ranges. `kunge2013-url-to-markdown` is the exception: it vendors the `kunge2013-fetch` runtime into `skills/kunge2013-url-to-markdown/scripts/lib/` so the published skill is self-contained and does not depend on the `kunge2013-fetch` npm package.

Current packages:
- `kunge2013-chrome-cdp` (Chrome CDP utilities), consumed by 5 skills (`kunge2013-danger-gemini-web`, `kunge2013-danger-x-to-markdown`, `kunge2013-post-to-wechat`, `kunge2013-post-to-weibo`, `kunge2013-post-to-x`)
- `kunge2013-md` (shared Markdown rendering and placeholder pipeline), consumed by 3 skills (`kunge2013-markdown-to-html`, `kunge2013-post-to-wechat`, `kunge2013-post-to-weibo`)
- `kunge2013-fetch` (URL-to-Markdown CLI), vendored into 1 skill (`kunge2013-url-to-markdown`)
...
2. Run the package build, e.g. `bun run --cwd packages/kunge2013-md build`
