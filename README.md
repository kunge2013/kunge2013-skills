...
Skills shared by kunge2013 for improving daily work efficiency with Claude Code.
...
This repository now supports publishing each `skills/kunge2013-*` directory as an individual ClawHub skill.
...
```bash
clawhub install kunge2013-imagine
clawhub install kunge2013-markdown-to-html
```
...
2. Select **kunge2013-skills**
3. Select the **kunge2013-skills** plugin
...
/plugin install kunge2013-skills@kunge2013-skills
```
...
|--------|-------------|----------|
| **kunge2013-skills** | Content generation, AI backends, and utility tools for daily work efficiency | All skills in this repository, organized below as Content Skills, AI Generation Skills, and Utility Skills |
...
/kunge2013-xhs-images posts/ai-future/article.md

# Specify style
/kunge2013-xhs-images posts/ai-future/article.md --style notion

# Specify layout
/kunge2013-xhs-images posts/ai-future/article.md --layout dense

# Combine style and layout
/kunge2013-xhs-images posts/ai-future/article.md --style notion --layout list

# Override palette
/kunge2013-xhs-images posts/ai-future/article.md --style notion --palette macaron

# Direct content input
/kunge2013-xhs-images 今日星座运势

# Non-interactive (skip all confirmations, for scheduled tasks)
/kunge2013-xhs-images posts/ai-future/article.md --yes
/kunge2013-xhs-images posts/ai-future/article.md --yes --preset knowledge-card
```
...
/kunge2013-infographic path/to/content.md

# Specify layout
/kunge2013-infographic path/to/content.md --layout pyramid

# Specify style (default: craft-handmade)
/kunge2013-infographic path/to/content.md --style technical-schematic

# Specify both
/kunge2013-infographic path/to/content.md --layout funnel --style corporate-memphis

# With aspect ratio (named preset or custom W:H)
/kunge2013-infographic path/to/content.md --aspect portrait
/kunge2013-infographic path/to/content.md --aspect 3:4
```
...
/kunge2013-diagram "how JWT authentication works"
/kunge2013-diagram "Kubernetes architecture" --type structural
/kunge2013-diagram "OAuth 2.0 flow"          --type sequence

# File path — skill reads, analyzes, and proposes a plan
/kunge2013-diagram path/to/article.md

# Language and output path
/kunge2013-diagram "微服务架构" --lang zh
/kunge2013-diagram "build pipeline" --out docs/build-pipeline.svg
```
...
/kunge2013-cover-image path/to/article.md

# Quick mode: skip confirmation, use auto-selection
/kunge2013-cover-image path/to/article.md --quick

# Specify dimensions (5D system)
/kunge2013-cover-image path/to/article.md --type conceptual --palette cool --rendering digital
/kunge2013-cover-image path/to/article.md --text title-subtitle --mood bold

# Style presets (backward-compatible shorthand)
/kunge2013-cover-image path/to/article.md --style blueprint

# Specify aspect ratio (default: 16:9)
/kunge2013-cover-image path/to/article.md --aspect 2.35:1

# Visual only (no title text)
/kunge2013-cover-image path/to/article.md --no-title
```
...
```bash
# From markdown file
/kunge2013-slide-deck path/to/article.md

# With style and audience
/kunge2013-slide-deck path/to/article.md --style corporate
/kunge2013-slide-deck path/to/article.md --audience executives

# Target slide count
/kunge2013-slide-deck path/to/article.md --slides 15

# Outline only (no image generation)
/kunge2013-slide-deck path/to/article.md --outline-only

# With language
/kunge2013-slide-deck path/to/article.md --lang zh
```
...
/kunge2013-comic posts/turing-story/source.md

# Specify art style and tone
/kunge2013-comic posts/turing-story/source.md --art manga --tone warm
/kunge2013-comic posts/turing-story/source.md --art ink-brush --tone dramatic

# Use preset (includes special rules)
/kunge2013-comic posts/turing-story/source.md --style ohmsha
/kunge2013-comic posts/turing-story/source.md --style wuxia

# Specify layout and aspect ratio
/kunge2013-comic posts/turing-story/source.md --layout cinematic
/kunge2013-comic posts/turing-story/source.md --aspect 16:9

# Specify language
/kunge2013-comic posts/turing-story/source.md --lang zh

# Direct content input
/kunge2013-comic "The story of Alan Turing and the birth of computer science"
```
...
/kunge2013-article-illustrator path/to/article.md --style vector-illustration --palette macaron
```
...
```bash
# Post with text
/kunge2013-post-to-x "Hello from Claude Code!"

# Post with images
/kunge2013-post-to-x "Check this out" --image photo.png

# Post X Article
/kunge2013-post-to-x --article path/to/article.md
```
...
```bash
/kunge2013-post-to-wechat 贴图 --markdown article.md --images ./photos/
/kunge2013-post-to-wechat 贴图 --markdown article.md --image img1.png --image img2.png --image img3.png
/kunge2013-post-to-wechat 贴图 --title "标题" --content "内容" --image img1.png --submit
```

**Article (文章)** - Full markdown/HTML with rich formatting:

```bash
/kunge2013-post-to-wechat 文章 --markdown article.md
/kunge2013-post-to-wechat 文章 --markdown article.md --theme grace
/kunge2013-post-to-wechat 文章 --html article.html
```
...
```bash
# Add to .kunge2013-skills/.env (project-level) or ~/.kunge2013-skills/.env (user-level)
...
```bash
mkdir -p .kunge2013-skills/kunge2013-post-to-wechat
```

Create `.kunge2013-skills/kunge2013-post-to-wechat/EXTEND.md`:
...
```bash
# Post with text
/kunge2013-post-to-weibo "Hello Weibo!"

# Post with images
/kunge2013-post-to-weibo "Check this out" --image photo.png

# Post with video
/kunge2013-post-to-weibo "Watch this" --video clip.mp4
```
...
```bash
# Publish article
/kunge2013-post-to-weibo --article article.md

# With cover image
/kunge2013-post-to-weibo --article article.md --cover cover.jpg
```
...
/kunge2013-imagine --prompt "A cute cat" --image cat.png

# With aspect ratio
/kunge2013-imagine --prompt "A landscape" --image landscape.png --ar 16:9

# High quality (2k)
/kunge2013-imagine --prompt "A banner" --image banner.png --quality 2k

# Specific provider
/kunge2013-imagine --prompt "A cat" --image cat.png --provider openai --model gpt-image-2

# Azure OpenAI (model = deployment name)
/kunge2013-imagine --prompt "A cat" --image cat.png --provider azure --model gpt-image-2

# OpenRouter
/kunge2013-imagine --prompt "A cat" --image cat.png --provider openrouter

# OpenRouter with reference images
/kunge2013-imagine --prompt "Make it blue" --image out.png --provider openrouter --model google/gemini-3.1-flash-image-preview --ref source.png

# DashScope (Aliyun Tongyi Wanxiang)
/kunge2013-imagine --prompt "一只可爱的猫" --image cat.png --provider dashscope

# DashScope with custom size
/kunge2013-imagine --prompt "为咖啡品牌设计一张 21:9 横幅海报，包含清晰中文标题" --image banner.png --provider dashscope --model qwen-image-2.0-pro --size 2048x872

# Z.AI GLM-Image
/kunge2013-imagine --prompt "一张带清晰中文标题的科技海报" --image out.png --provider zai

# MiniMax
/kunge2013-imagine --prompt "A fashion editorial portrait by a bright studio window" --image out.jpg --provider minimax

# MiniMax with subject reference
/kunge2013-imagine --prompt "A girl stands by the library window, cinematic lighting" --image out.jpg --provider minimax --model image-01 --ref portrait.png --ar 16:9

# Replicate (default: google/nano-banana-2)
/kunge2013-imagine --prompt "A cat" --image cat.png --provider replicate

# Replicate Seedream 4.5
/kunge2013-imagine --prompt "A studio portrait" --image portrait.png --provider replicate --model bytedance/seedream-4.5 --ar 3:2

# Replicate Wan 2.7 Image Pro
/kunge2013-imagine --prompt "A concept frame" --image frame.png --provider replicate --model wan-video/wan-2.7-image-pro --size 2048x1152

# Jimeng (即梦)
/kunge2013-imagine --prompt "一只可爱的猫" --image cat.png --provider jimeng

# Seedream (豆包)
/kunge2013-imagine --prompt "一只可爱的猫" --image cat.png --provider seedream

# With reference images (Google, OpenAI, Azure OpenAI, OpenRouter, Replicate, MiniMax, or Seedream 5.0/4.5/4.0)
/kunge2013-imagine --prompt "Make it blue" --image out.png --ref source.png

# Batch mode
/kunge2013-imagine --batchfile batch.json --jobs 4 --json
```
...
| `kunge2013_IMAGE_GEN_MAX_WORKERS` | Override batch worker cap | `10` |
| `kunge2013_IMAGE_GEN_<PROVIDER>_CONCURRENCY` | Override provider concurrency | provider-specific |
| `kunge2013_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS` | Override provider request start gap | provider-specific |
...
- Replicate defaults to `google/nano-banana-2`. `kunge2013-imagine` only enables Replicate advanced options for `google/nano-banana*`, `bytedance/seedream-4.5`, `bytedance/seedream-5-lite`, `wan-video/wan-2.7-image`, and `wan-video/wan-2.7-image-pro`.
...
```bash
/kunge2013-danger-gemini-web "Hello, Gemini"
/kunge2013-danger-gemini-web --prompt "Explain quantum computing"
```

**Image Generation:**

```bash
/kunge2013-danger-gemini-web --prompt "A cute cat" --image cat.png
/kunge2013-danger-gemini-web --promptfiles system.md content.md --image out.png
```
...
/kunge2013-youtube-transcript https://www.youtube.com/watch?v=VIDEO_ID

# Specify languages (priority order)
/kunge2013-youtube-transcript https://youtu.be/VIDEO_ID --languages zh,en,ja

# With chapters and speaker identification
/kunge2013-youtube-transcript https://youtu.be/VIDEO_ID --chapters --speakers

# SRT subtitle format
/kunge2013-youtube-transcript https://youtu.be/VIDEO_ID --format srt

# List available transcripts
/kunge2013-youtube-transcript https://youtu.be/VIDEO_ID --list
```
...
/kunge2013-url-to-markdown https://example.com/article

# Wait mode - for login-required pages
/kunge2013-url-to-markdown https://example.com/private --wait

# Save to specific file
/kunge2013-url-to-markdown https://example.com/article -o output.md
```
...
#### kunge2013-danger-x-to-markdown

Converts X (Twitter) content to markdown format. Supports tweet threads and X Articles.

```bash
# Convert tweet to markdown
/kunge2013-danger-x-to-markdown https://x.com/username/status/123456

# Save to specific file
/kunge2013-danger-x-to-markdown https://x.com/username/status/123456 -o output.md

# JSON output
/kunge2013-danger-x-to-markdown https://x.com/username/status/123456 --json

# Download media (images/videos) to local files
/kunge2013-danger-x-to-markdown https://x.com/username/status/123456 --download-media
```
...
#### kunge2013-compress-image

Compress images to reduce file size while maintaining quality.

```bash
/kunge2013-compress-image path/to/image.png
/kunge2013-compress-image path/to/images/ --quality 80
```

#### kunge2013-format-markdown

Format plain text or markdown files with proper frontmatter, titles, summaries, headings, bold, lists, and code blocks.

```bash
# Format a markdown file
/kunge2013-format-markdown path/to/article.md

# Format with specific output
/kunge2013-format-markdown path/to/draft.md
```
...
```bash
# Basic conversion
/kunge2013-markdown-to-html article.md

# Theme + color
/kunge2013-markdown-to-html article.md --theme grace --color red

# Convert ordinary external links to bottom citations
/kunge2013-markdown-to-html article.md --cite
```
...
1. CLI environment variables (e.g., `OPENAI_API_KEY=xxx /kunge2013-imagine ...`)
2. `process.env` (system environment)
3. `<cwd>/.kunge2013-skills/.env` (project-level)
4. `~/.kunge2013-skills/.env` (user-level)

**Setup**:

```bash
# Create user-level config directory
mkdir -p ~/.kunge2013-skills

# Create .env file
cat > ~/.kunge2013-skills/.env << 'EOF'
# OpenAI
...
```bash
mkdir -p .kunge2013-skills
# Add .kunge2013-skills/.env to .gitignore to avoid committing secrets
echo ".kunge2013-skills/.env" >> .gitignore
```
...
1. `.kunge2013-skills/<skill-name>/EXTEND.md` - Project-level (for team/project-specific settings)
2. `~/.kunge2013-skills/<skill-name>/EXTEND.md` - User-level (for personal preferences)

**Example**: To customize `kunge2013-cover-image` with your brand colors:

```bash
mkdir -p .kunge2013-skills/kunge2013-cover-image
```

Then create `.kunge2013-skills/kunge2013-cover-image/EXTEND.md`:
...
```bash
HTTP_PROXY=http://127.0.0.1:7890 HTTPS_PROXY=http://127.0.0.1:7890 /kunge2013-danger-gemini-web "Hello"
```
