...
test("findChromeExecutable prefers env overrides and falls back to candidate paths", async (t) => {
  const root = await makeTempDir("kunge2013-chrome-bin-");
  ...
  useEnv(t, { kunge2013_CHROME_PATH: envChrome });
  assert.equal(
    findChromeExecutable({
      envNames: ["kunge2013_CHROME_PATH"],
      candidates: { default: [fallbackChrome] },
    }),
    envChrome,
  );

  useEnv(t, { kunge2013_CHROME_PATH: null });
  assert.equal(
    findChromeExecutable({
      envNames: ["kunge2013_CHROME_PATH"],
      candidates: { default: [fallbackChrome] },
...
test("resolveSharedChromeProfileDir supports env overrides, WSL paths, and default suffixes", (t) => {
  useEnv(t, { kunge2013_SHARED_PROFILE: "/tmp/custom-profile" });
  assert.equal(
    resolveSharedChromeProfileDir({
      envNames: ["kunge2013_SHARED_PROFILE"],
  ...
  useEnv(t, { kunge2013_SHARED_PROFILE: null });
  assert.equal(
    resolveSharedChromeProfileDir({
...
test("findExistingChromeDebugPort reads DevToolsActivePort and validates it against a live endpoint", async (t) => {
  const root = await makeTempDir("kunge2013-cdp-profile-");
  t.after(() => fs.rm(root, { recursive: true, force: true }));
...
test("discoverRunningChromeDebugPort reads DevToolsActivePort from the provided user-data dir", async (t) => {
  const root = await makeTempDir("kunge2013-cdp-user-data-");
  t.after(() => fs.rm(root, { recursive: true, force: true }));
