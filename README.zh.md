...
现在这个仓库支持把每个 `skills/kunge2013-*` 目录作为独立 ClawHub skill 发布。
...
2. 选择 **kunge2013-skills**
3. 选择 **kunge2013-skills** 插件
4. 选择 **Install now**
...
| 插件 | 说明 | 包含内容 |
|------|------|----------|
| **kunge2013-skills** | 提供内容生成、AI 后端和日常效率工具技能 | 仓库中的全部 skills，仍按下方的内容技能、AI 生成技能、工具技能三个分类展示 |
...
```bash
# 主题描述 —— 技能分析并提出方案
/kunge2013-diagram "JWT 认证流程是怎么工作的"
/kunge2013-diagram "Kubernetes 架构" --type structural
/kunge2013-diagram "OAuth 2.0 流程"  --type sequence
...
# 直接输入内容
/kunge2013-comic "图灵的故事与计算机科学的诞生"
```
...
# 发布带图片
/kunge2013-post-to-x "看看这个" --image photo.png
...
```bash
# 添加到 .kunge2013-skills/.env（项目级）或 ~/.kunge2013-skills/.env（用户级）
...
创建 `.kunge2013-skills/kunge2013-post-to-wechat/EXTEND.md`：
...
每个账号拥有独立的 Chrome 配置目录，保证浏览器方式下的登录会话互不干扰。API 凭证可在 EXTEND.md 中直接配置，也可通过 `.env` 文件使用别名前缀的环境变量（如 `WECHAT_kunge2013_APP_ID`）。
...
# 发布带图片
/kunge2013-post-to-weibo "看看这个" --image photo.png

# 发布带视频
/kunge2013-post-to-weibo "看这个" --video clip.mp4
```
...
```bash
# 基础生成（自动检测服务商）
/kunge2013-imagine --prompt "一只可爱的猫" --image cat.png

# 指定宽高比
/kunge2013-imagine --prompt "风景图" --image landscape.png --ar 16:9

# 高质量（2k 分辨率）
/kunge2013-imagine --prompt "横幅图" --image banner.png --quality 2k

# 指定服务商
/kunge2013-imagine --prompt "一只猫" --image cat.png --provider openai --model gpt-image-2

# Azure OpenAI（model 为部署名称）
/kunge2013-imagine --prompt "一只猫" --image cat.png --provider azure --model gpt-image-2

# OpenRouter
/kunge2013-imagine --prompt "一只猫" --image cat.png --provider openrouter

# OpenRouter + 参考图
/kunge2013-imagine --prompt "把它变成蓝色" --image out.png --provider openrouter --model google/gemini-3.1-flash-image-preview --ref source.png
...
/kunge2013-imagine --prompt "一只猫" --image cat.png --provider replicate

# Replicate Seedream 4.5
/kunge2013-imagine --prompt "一张影棚人像" --image portrait.png --provider replicate --model bytedance/seedream-4.5 --ar 3:2

# Replicate Wan 2.7 Image Pro
/kunge2013-imagine --prompt "一张概念分镜" --image frame.png --provider replicate --model wan-video/wan-2.7-image-pro --size 2048x1152
...
/kunge2013-imagine --prompt "把它变成蓝色" --image out.png --ref source.png
...
| `kunge2013_IMAGE_GEN_MAX_WORKERS` | 批量模式最大 worker 数 | `10` |
| `kunge2013_IMAGE_GEN_<PROVIDER>_CONCURRENCY` | 覆盖 provider 并发数 | provider 默认值 |
| `kunge2013_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS` | 覆盖 provider 请求启动间隔 | provider 默认值 |
...
- Replicate 默认模型改为 `google/nano-banana-2`。`kunge2013-imagine` 目前只对 `google/nano-banana*`、`bytedance/seedream-4.5`、`bytedance/seedream-5-lite`、`wan-video/wan-2.7-image` 和 `wan-video/wan-2.7-image-pro` 开启本地能力识别与校验。
...
```bash
/kunge2013-danger-gemini-web "你好，Gemini"
/kunge2013-danger-gemini-web --prompt "解释量子计算"
```

**图片生成：**

```bash
/kunge2013-danger-gemini-web --prompt "一只可爱的猫" --image cat.png
...
**加载优先级**（高优先级覆盖低优先级）：
1. 命令行环境变量（如 `OPENAI_API_KEY=xxx /kunge2013-imagine ...`）
2. `process.env`（系统环境变量）
3. `<cwd>/.kunge2013-skills/.env`（项目级）
4. `~/.kunge2013-skills/.env`（用户级）
...
```bash
mkdir -p .kunge2013-skills
# 将 .kunge2013-skills/.env 添加到 .gitignore 避免提交密钥
...
**扩展路径**（按优先级检查）：
1. `.kunge2013-skills/<skill-name>/EXTEND.md` - 项目级（团队/项目特定设置）
2. `~/.kunge2013-skills/<skill-name>/EXTEND.md` - 用户级（个人偏好设置）

**示例**：为 `kunge2013-cover-image` 自定义品牌配色：

```bash
mkdir -p .kunge2013-skills/kunge2013-cover-image
```

然后创建 `.kunge2013-skills/kunge2013-cover-image/EXTEND.md`：
...
```bash
HTTP_PROXY=http://127.0.0.1:7890 HTTPS_PROXY=http://127.0.0.1:7890 /kunge2013-danger-gemini-web "你好"
```
