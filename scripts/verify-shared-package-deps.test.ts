...
test("verify-shared-package-deps accepts npm semver dependencies", async (t) => {
  const root = await makeTempDir("kunge2013-verify-shared-ok-");
  t.after(() => fs.rm(root, { recursive: true, force: true }));

  await writeJson(path.join(root, "packages", "kunge2013-md", "package.json"), {
    name: "kunge2013-md",
    version: "1.2.3",
  });
...
test("verify-shared-package-deps rejects file dependencies and vendored workspace packages", async (t) => {
  const root = await makeTempDir("kunge2013-verify-shared-bad-");
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  ...
  await writeJson(path.join(root, "skills", "demo", "scripts", "package.json"), {
    dependencies: {
      "kunge2013-md": "file:./vendor/kunge2013-md",
    },
  });
  await writeFile(path.join(root, "skills", "demo", "scripts", "vendor", "kunge2013-md", "index.ts"));
