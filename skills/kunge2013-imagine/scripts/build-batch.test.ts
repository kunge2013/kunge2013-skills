...
const scriptPath = path.join(repoRoot, "skills", "kunge2013-imagine", "scripts", "build-batch.ts");
...
async function makeFixture(): Promise<{
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "kunge2013-imagine-build-batch-"));
...
test("build-batch omits default model so kunge2013-imagine can resolve env or EXTEND defaults", async () => {
