...
Environment variables:
  ...
  kunge2013_IMAGE_GEN_<PROVIDER>_CONCURRENCY  Override provider concurrency
  kunge2013_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS  Override provider start gap in ms

Env file load order: CLI args > EXTEND.md > process.env > <cwd>/.kunge2013-skills/.env > ~/.kunge2013-skills/.env`);
}
...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".kunge2013-skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".kunge2013-skills", ".env"));
...
function getExtendConfigPathPairs(cwd: string, home: string): ExtendConfigPathPair[] {
  return [
    {
      current: path.join(cwd, ".kunge2013-skills", "kunge2013-imagine", "EXTEND.md"),
      legacy: path.join(cwd, ".kunge2013-skills", "kunge2013-image-gen", "EXTEND.md"),
    },
    {
      current: path.join(home, ".kunge2013-skills", "kunge2013-imagine", "EXTEND.md"),
      legacy: path.join(home, ".kunge2013-skills", "kunge2013-image-gen", "EXTEND.md"),
    },
  ];
}
...
export function getConfiguredMaxWorkers(extendConfig: Partial<ExtendConfig>): number {
  const envValue = parsePositiveInt(process.env.kunge2013_IMAGE_GEN_MAX_WORKERS);
...
export function getConfiguredProviderRateLimits(
  ...
  for (const provider of ["replicate", "google", "openai", "openrouter", "dashscope", "zai", "minimax", "jimeng", "seedream", "azure"] as Provider[]) {
    const envPrefix = `kunge2013_IMAGE_GEN_${provider.toUpperCase()}`;
...
export function detectProvider(args: CliArgs): Provider {
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, AZURE_OPENAI_API_KEY+AZURE_OPENAI_BASE_URL, OPENROUTER_API_KEY, DASHSCOPE_API_KEY, ZAI_API_KEY, MINIMAX_API_KEY, REPLICATE_API_TOKEN, JIMENG keys, or ARK_API_KEY.\n" +
      "Create ~/.kunge2013-skills/.env or <cwd>/.kunge2013-skills/.env with your keys."
  );
}
