...
test("parseArgs parses the main kunge2013-imagine CLI flags", () => {
...
test("loadExtendConfig renames legacy EXTEND.md when the new path is missing", async () => {
  const root = await makeTempDir("kunge2013-imagine-extend-");
  const cwd = path.join(root, "project");
  const home = path.join(root, "home");
  const legacyPath = path.join(cwd, ".kunge2013-skills", "kunge2013-image-gen", "EXTEND.md");
  const currentPath = path.join(cwd, ".kunge2013-skills", "kunge2013-imagine", "EXTEND.md");
...
test("loadExtendConfig leaves legacy EXTEND.md untouched when both paths exist", async () => {
  const root = await makeTempDir("kunge2013-imagine-extend-dual-");
  const cwd = path.join(root, "project");
  const home = path.join(root, "home");
  const legacyPath = path.join(cwd, ".kunge2013-skills", "kunge2013-image-gen", "EXTEND.md");
  const currentPath = path.join(cwd, ".kunge2013-skills", "kunge2013-imagine", "EXTEND.md");
...
test("batch worker and provider-rate-limit configuration prefer env over EXTEND config", (t) => {
  useEnv(t, {
    kunge2013_IMAGE_GEN_MAX_WORKERS: "12",
    kunge2013_IMAGE_GEN_GOOGLE_CONCURRENCY: "5",
    kunge2013_IMAGE_GEN_GOOGLE_START_INTERVAL_MS: "450",
    kunge2013_IMAGE_GEN_ZAI_CONCURRENCY: "4",
  });
...
test("loadBatchTasks and createTaskArgs resolve batch-relative paths", async (t) => {
  const root = await makeTempDir("kunge2013-imagine-batch-");
...
test("path normalization, worker count, and retry classification follow expected rules", () => {
  ...
  assert.equal(
    isRetryableGenerationError(
      new Error("Replicate returned 2 outputs, but kunge2013-imagine currently supports saving exactly one image per request."),
    ),
    false,
  );
  assert.equal(
  ...
  assert.equal(
    isRetryableGenerationError(
      new Error("DashScope wan2.7 image models in kunge2013-imagine support exactly one output image per request."),
    ),
    false,
  );
  assert.equal(
