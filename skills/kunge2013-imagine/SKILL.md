...
| Path | Scope |
|------|-------|
| `.kunge2013-skills/kunge2013-imagine/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/kunge2013-skills/kunge2013-imagine/EXTEND.md` | XDG |
| `$HOME/.kunge2013-skills/kunge2013-imagine/EXTEND.md` | User home |
...
Legacy compatibility: if `.kunge2013-skills/kunge2013-image-gen/EXTEND.md` exists and the new path doesn't, the runtime renames it to `kunge2013-imagine`. If both exist, the runtime leaves them alone and uses the new path.
...
| `kunge2013_IMAGE_GEN_MAX_WORKERS` | Override batch worker cap |
| `kunge2013_IMAGE_GEN_<PROVIDER>_CONCURRENCY` | Per-provider concurrency (e.g., `kunge2013_IMAGE_GEN_REPLICATE_CONCURRENCY`) |
| `kunge2013_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS` | Per-provider start-gap |

**Load priority**: CLI args > EXTEND.md > env vars > `<cwd>/.kunge2013-skills/.env` > `~/.kunge2013-skills/.env`
...
| Input is `outline.md` + `prompts/` (e.g. from `kunge2013-article-illustrator`) | Batch — use `scripts/build-batch.ts` to assemble the payload | The outline + prompt files already contain everything needed |
