...
description: First-time setup and default model selection flow for kunge2013-image-gen
---
...
| Choice | Path | Scope |
|--------|------|-------|
| Project | `.kunge2013-skills/kunge2013-image-gen/EXTEND.md` | Current project |
| User | `$HOME/.kunge2013-skills/kunge2013-image-gen/EXTEND.md` | All projects |
...
- In `kunge2013-image-gen`, Azure `--model` / `default_model.azure` should be the Azure deployment name, not just the underlying model family.
...
- In `kunge2013-image-gen`, `quality` is a compatibility preset. It is not a native DashScope parameter.
...
- Z.AI does not accept reference images or `n > 1` in `kunge2013-image-gen`; use Google/OpenAI providers for those workflows.
