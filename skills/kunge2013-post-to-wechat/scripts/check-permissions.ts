...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.kunge2013-skills', '.env');
  const userEnv = path.join(os.homedir(), '.kunge2013-skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== kunge2013-post-to-wechat: Permission & Environment Check ===\n');
