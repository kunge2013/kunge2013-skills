...
2. `<cwd>/.kunge2013-skills/.env` with `WECHAT_APP_ID=...`
3. `$HOME/.kunge2013-skills/.env` with `WECHAT_APP_ID=...`
...
Where to save?
A) Project-level: .kunge2013-skills/.env (this project only)
B) User-level: ~/.kunge2013-skills/.env (all projects)
```
