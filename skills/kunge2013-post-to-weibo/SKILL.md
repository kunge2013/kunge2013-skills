...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.kunge2013-skills/kunge2013-post-to-weibo/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/kunge2013-skills/kunge2013-post-to-weibo/EXTEND.md` | XDG |
| 3 | `$HOME/.kunge2013-skills/kunge2013-post-to-weibo/EXTEND.md` | User home |
...
If a script fails with `Chrome debug port not ready` or `Unable to connect`, kill only the CDP Chrome instances (those with `--remote-debugging-port` AND the kunge2013-skills profile), then retry:

```bash
pkill -f "remote-debugging-port.*kunge2013-skills/chrome-profile" 2>/dev/null; sleep 2
```

**CRITICAL**: Never kill all Chrome processes (`pkill -f "Google Chrome"`). Only kill Chrome instances launched by CDP with the kunge2013-skills profile directory. The user may have regular Chrome windows open.
