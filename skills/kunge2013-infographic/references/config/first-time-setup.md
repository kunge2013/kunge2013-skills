...
description: First-time setup flow for kunge2013-infographic preferences
---
...
options:
  - label: "Project"
    description: ".kunge2013-skills/ (this project only)"
  - label: "User"
    description: "~/.kunge2013-skills/ (all projects)"
```
...
| Choice | Path | Scope |
|--------|------|-------|
| Project | `.kunge2013-skills/kunge2013-infographic/EXTEND.md` | Current project |
| User | `~/.kunge2013-skills/kunge2013-infographic/EXTEND.md` | All projects |

XDG path (`${XDG_CONFIG_HOME:-$HOME/.config}/kunge2013-skills/kunge2013-infographic/EXTEND.md`) is also recognized at read time but not offered as a save target during first-time setup.
...
`preferred_image_backend: auto` is the baked-in default — first-time setup never asks about it. The `## Image Generation Tools` rule in SKILL.md then picks the runtime-native tool (Codex `imagegen`, Hermes `image_generate`, etc.) when one is available, and falls back to installed backends like `kunge2013-imagine`.
