...
Fetches any URL via `kunge2013-fetch` CLI (Chrome CDP + site-specific adapters) and converts it to clean markdown.
...
4. `${READER}` = `{baseDir}/scripts/kunge2013-fetch`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.kunge2013-skills/kunge2013-url-to-markdown/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/kunge2013-skills/kunge2013-url-to-markdown/EXTEND.md` | XDG |
| 3 | `$HOME/.kunge2013-skills/kunge2013-url-to-markdown/EXTEND.md` | User home |
...
- **Q3 — Save** (header "Save"): "Where to save preferences?"
  - "User (Recommended)" — `~/.kunge2013-skills/` (all projects)
  - "Project" — `.kunge2013-skills/` (this project only)
...
| `--chrome-profile-dir <path>` | Chrome user data directory (default: `kunge2013_CHROME_PROFILE_DIR` env or `./kunge2013-skills/chrome-profile`) |
...
The agent must construct the output file path — `kunge2013-fetch` does not auto-generate paths.
...
|----------|-------------|
| `kunge2013_CHROME_PROFILE_DIR` | Chrome user data directory (can also use `--chrome-profile-dir`) |
